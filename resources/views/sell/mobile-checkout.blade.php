{{-- resources/views/sell/mobile-checkout.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Checkout – {{ $evaluation->model->name ?? 'Mobile' }}</title>
...
<div class="co-wrap">
    ...
    <div class="co-grid">
        <div>
            <form method="POST" action="{{ route('sell.phone.checkout.submit', $evaluation->id) }}" id="checkoutForm">
                @csrf
                ...
                <div class="co-panel">
                    ...
                    <div class="co-panel-body">
                        <div class="slot-grid" id="slotGrid">
                            @php
                                $slots = [
                                    ['day' => 'Today',    'time' => '10:00 AM – 1:00 PM', 'val' => 'Today 10:00 AM – 1:00 PM'],
                                    ['day' => 'Today',    'time' => '2:00 PM – 6:00 PM',  'val' => 'Today 2:00 PM – 6:00 PM'],
                                    ['day' => 'Tomorrow', 'time' => '10:00 AM – 1:00 PM', 'val' => 'Tomorrow 10:00 AM – 1:00 PM'],
                                    ['day' => 'Tomorrow', 'time' => '2:00 PM – 6:00 PM',  'val' => 'Tomorrow 2:00 PM – 6:00 PM'],
                                ];
                            @endphp
                            @foreach($slots as $slot)
                            <div class="slot-card" data-val="{{ $slot['val'] }}" onclick="selectSlot(this, '{{ $slot['val'] }}')">
                ...
                <button type="submit" class="co-continue-btn" id="confirmOrderBtn" disabled>CONFIRM ORDER →</button>
            ...
            <div class="co-price-card">
                <div class="co-price-top">
                    <div class="co-price-img">
                        @if($evaluation->model && $evaluation->model->image)
                            <img src="{{ asset('storage/'.$evaluation->model->image) }}" alt="">
                        @else
                            <span style="font-size:24px;opacity:.3;">📱</span>
                        @endif
                    </div>
                    <div>
                        <div class="co-price-name">{{ $evaluation->model->name ?? 'Mobile' }}</div>
                        <div class="co-price-spec">
                            {{ $evaluation->brand->name ?? '' }}
                            @if($evaluation->variant) · {{ $evaluation->variant->memory ?? $evaluation->variant->name ?? '' }} @endif
                        </div>
                    </div>
                </div>
                <div class="co-price-body">
                    ...
                    <div class="co-price-row">
                        ...
                        <span class="val">₹{{ number_format((float) $evaluation->estimated_price) }}</span>
                    </div>
                </div>
...
function checkReady() {
    ...
    const flatNo = document.querySelector('input[name="flat_no"]').value.trim();
    const locality = document.querySelector('input[name="locality"]').value.trim();
    const city = document.getElementById('cityInput').value.trim();
    const btn = document.getElementById('confirmOrderBtn');

    const isReady = slot.length > 0 && pincode.length === 6 && flatNo.length > 0 && locality.length > 0 && city.length > 0;
    ...
    navigator.geolocation.getCurrentPosition(pos => {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
            ...
            .then(data => {
                ...
                if (addr.city || addr.town || addr.village || addr.district) {
                    document.getElementById('cityInput').value = addr.city || addr.town || addr.village || addr.district;
                }
                checkReady();
            });
...
window.onload = function() {
    ...
    if (firstSlot) {
        ...
        document.getElementById('pickup_slot_input').value = firstSlot.getAttribute('data-val');
    }
    checkReady();
};
</script>
